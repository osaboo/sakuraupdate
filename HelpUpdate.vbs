...
Sub Main()
	...
	TraceOut "ヘルプファイルのリリース済みバージョンを確認します。"
    ...
    'TraceOut "サクラエディタのヘルプバージョン:" & wnewver

    'if wnewver <= wcurver Then
        If  MessageBox("ヘルプファイルを更新しますか?",4) = 7 then 
	        Exit Sub
	    End if
    'End If
	Editor.ActivateWinOutput
...
if false then
    ...
    ' sakura.chmの展開
    If fs.FileExists(WorkDir & "\sakura.chm") Then
        fs.DeleteFile (WorkDir & "\sakura.chm")
    End If
    ...
    sakurachm = Editor.ExpandParameter("$S")
    sakurachm = Replace(sakurachm, "sakura.exe", "sakura.chm")
    sakuraold = sakurachm & ".old"
    ...
    wcmd = wcmd & vbCrLf & "ren """ & sakurachm & """ sakura.chm.old"
    wcmd = wcmd & vbCrLf & "if exist """ & sakurachm & """ goto :L1"
    wcmd = wcmd & vbCrLf & "copy /y """ & WorkDir & "\sakura.chm"" """ & sakurachm & """"
    'wcmd = wcmd & vbCrLf & """C:\Program Files (x86)\sakura\sakura.exe"""
    'wcmd = wcmd & vbCrLf & "start """" """ & sakurachm & """"
    wcmd = wcmd & vbCrLf & "timeout /t 3"
...
' Get SourceForge RSS 
function GetSFRSS(byref aurl, apath, atargetver)
    Dim rCode
    Dim Itemtitle
    Dim i
    Dim wlink
    ...
    Editor.TraceOut "Version " & atargetver & "のヘルプファイルを探します..."
